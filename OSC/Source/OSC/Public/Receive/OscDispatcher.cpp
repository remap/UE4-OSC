...
void UOscDispatcher::Listen(FIPv4Address address, uint32_t port, bool multicastLoopback)
{
    if(_listening != std::make_pair(address, port))
    {
        ...
        if(_socket)
        {
            ...
            UE_LOG(LogUE4OSC, Display, TEXT("Listen to port %d"), port);
        }
        else
        {
            UE_LOG(LogUE4OSC, Warning, TEXT("Cannot listen port %d"), port);
        }
    }
}
...
{
    UE_LOG(LogUE4OSC, Display, TEXT("Stop listening"));
                        ...
                        const FIPv4Address & senderIp)
{
    ...
    {
        UE_LOG(LogUE4OSC, Warning, TEXT("OSC Received Message Error: %s"), osc::errorString(message.State()));
        return;
    }
    ...
    for(auto it = argBegin; it != argEnd; ++it)
    {
        ...
        else if(it->IsBlob())
        {
            ...
            {
                UE_LOG(LogUE4OSC, Warning, TEXT("OSC Received Message Error: %s"), osc::errorString(error));
            }
    ...
    if(!added)
    {
        UE_LOG(LogUE4OSC, Warning, TEXT("Circular Buffer Full: Message Ignored"));
    }
}
                       ...
                       const FIPv4Address & senderIp)
{
    ...
    {
        UE_LOG(LogUE4OSC, Warning, TEXT("OSC Received Bundle Error: %s"), osc::errorString(bundle.State()));
        return;
    }
...
void UOscDispatcher::Callback(const FArrayReaderPtr& data, const FIPv4Endpoint& endpoint)
{
    ...
    {
        UE_LOG(LogUE4OSC, Warning, TEXT("OSC Received Packet Error: %s"), osc::errorString(packet.State()));
        return;
    }
...
#if !NO_LOGGING
    // Log received packet
    if(!LogUE4OSC.IsSuppressed(ELogVerbosity::Verbose))
    {
        const auto encoded = FBase64::Encode(*data);
        UE_LOG(LogUE4OSC, Verbose, TEXT("Received: %s"), *encoded);
    }
#endif
}
